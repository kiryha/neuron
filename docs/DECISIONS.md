...
| D-025 | Accepted | 2026-09-03 | Build the next application slice as a local-only Three.js viewer of the hero's smooth world-space normal pass, with an orbitable camera and a reset to a stable app-authored reference view. | The displayed RGB preview maps float normals from `[-1, 1]` to `[0, 1]`. Prompt UI, model inference, `P`/`V`/Coverage passes, FastAPI integration, and Hugging Face deployment remain later work. The reference camera lives in application code and is not a pixel-exact Houdini calibration claim. |
