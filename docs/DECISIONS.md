...
| D-004 | Accepted | 2026-08-26 | Camera and fixed-surface context are part of the learning problem. | Store the required context directly in each EXR as `P`, unbumped `Nb`, `V`, and Beauty alpha Coverage; separate camera metadata is not required for dataset v0. |
...
| D-025 | Accepted | 2026-09-03 | Build the next application slice as a local-only Three.js viewer of the hero's smooth world-space normal pass, with an orbitable camera and a reset to a stable app-authored reference view. | The displayed RGB preview maps float normals from `[-1, 1]` to `[0, 1]`. A visual prompt field may accept text but has no behavior yet; prompt processing, model inference, `P`/`V`/Coverage passes, FastAPI integration, and Hugging Face deployment remain later work. The reference camera lives in application code and is not a pixel-exact Houdini calibration claim. |
